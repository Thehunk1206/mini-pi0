...
class IsaacLabAdapter(SimulatorAdapter):
    """Scaffold adapter for IsaacLab backend integration."""
    ...
    def __init__(self, cfg: RootConfig):
        ...
        self.cfg = cfg
        self._has_module = importlib.util.find_spec("isaaclab") is not None
    ...
    def _raise(self):
        ...
        if not self._has_module:
            raise RuntimeError(
                "IsaacLab adapter is scaffolded. IsaacLab is not installed in this environment."
            )
        raise NotImplementedError(
            "IsaacLab adapter is scaffolded for extension. "
            "Implement reset/step/render mappings for your target task/robot."
        )
