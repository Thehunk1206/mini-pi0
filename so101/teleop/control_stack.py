"""Input-agnostic SO-101 Ruckig OTG, profiles, and tracking safety."""
...
from so101.teleop.profiles import PROFILE_LIMIT_SCALES, PROFILE_NAMES
...
JOINT_LIMIT_NUMERICAL_TOLERANCE_DEG = 1e-3
JOINT_LIMIT_SATURATION_TOLERANCE_DEG = 0.05
...
class SO101ControlStack:
    """Own the shared live OTG, profile, gripper, and tracking-fault state."""
    ...
    def __init__(
        ...
        *,
        ...
        max_target_step_deg: tuple[float, ...] | None = None,
    ) -> None:
        ...
        self.max_target_step_deg = (
            None
            if max_target_step_deg is None
            else np.asarray(max_target_step_deg, dtype=float)
        )
        if self.max_target_step_deg is not None and (
            self.max_target_step_deg.shape != (len(self.arm_joint_names),)
            or not np.all(np.isfinite(self.max_target_step_deg))
            or np.any(self.max_target_step_deg <= 0.0)
        ):
            raise ValueError("max_target_step_deg must contain five positive values")
    ...
    def _empty_snapshot(self) -> dict[str, Any]:
        return {
            ...
            "ruckig": {
                ...
                "target_velocity": {},
                "result": "Finished", "status": "uninitialized", "error": None,
            },
    ...
    def set_profile(self, profile: str) -> None:
        if self.hold_active:
            raise HoldActiveError("Release the motion clutch before changing the motion profile")
    ...
    def _target_vector(self, target: dict[str, Any]) -> tuple[np.ndarray | None, str | None]:
        ...
        for index, (name, value) in enumerate(
            zip(self.joint_names, values, strict=True)
        ):
            lower, upper = self.joint_limits[name]
            if (
                value < lower - JOINT_LIMIT_NUMERICAL_TOLERANCE_DEG
                or value > upper + JOINT_LIMIT_NUMERICAL_TOLERANCE_DEG
            ):
                return None, f"{name} target {value:.3f} is outside [{lower:.3f}, {upper:.3f}]"
            # Numerical IK can return a boundary value a few millionths of a
            # degree beyond the same URDF limit it used. Accept and normalize
            # only that round-off-sized excess; genuine violations stay rejected.
            values[index] = float(np.clip(value, lower, upper))

        saturated_arm_joints = []
        for index, name in enumerate(self.arm_joint_names):
            lower, upper = self.joint_limits[name]
            if min(abs(values[index] - lower), abs(values[index] - upper)) <= (
                JOINT_LIMIT_SATURATION_TOLERANCE_DEG
            ):
                saturated_arm_joints.append(name)
        if len(saturated_arm_joints) >= 2:
            return None, (
                "IK target saturates multiple joint limits: "
                + ", ".join(saturated_arm_joints)
            )

        if self.max_target_step_deg is not None and self._last_valid_target is not None:
            target_step = np.abs(values[:5] - self._last_valid_target[:5])
            excessive = np.flatnonzero(target_step > self.max_target_step_deg)
            if len(excessive):
                details = ", ".join(
                    f"{self.arm_joint_names[index]}={target_step[index]:.1f}°"
                    for index in excessive
                )
                return None, f"IK target jump rejected: {details}"
        return values, None
    ...
    def step(
        ...
        *,
        ...
        otg_error: str | None = None
        otg_fault = False
        arm_result_text = "Finished"
        if not self.paused_fault:
            ...
            try:
                ...
                arm_result_text = self._result_name(arm_result)
            except (RuckigError, RuntimeError) as exc:
                # A synchronized moving-target solve can occasionally fail near
                # a limit. Retry the same bounded position with zero terminal
                # velocity before pausing the controller.
                otg_error = str(exc)
                target_velocity = np.zeros(len(self.arm_joint_names), dtype=float)
                try:
                    arm_result = self._arm.step(
                    ...
                    arm_result_text = self._result_name(arm_result)
                    status = "tracking_zero_velocity_fallback"
                except (RuckigError, RuntimeError) as fallback_exc:
                    otg_error = f"{otg_error}; zero-velocity retry: {fallback_exc}"
                    otg_fault = True
                    self.paused_fault = True
                    self._arm.reset(measured_vector[:5])
                    self._last_valid_target = measured_vector.copy()
                    ...
                    self._previous_velocity_target = None
                    self._filtered_target_velocity.fill(0.0)
                    arm_result_text = "Error"
                    status = "paused_otg_error"
        else:
            target_velocity = np.zeros(len(self.arm_joint_names), dtype=float)
            arm_result_text = self._result_name(Result.Finished)
